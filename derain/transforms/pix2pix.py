...
class Scale(object):
  ...
  def __call__(self, imgA, imgB):
    ...
    for img in imgs:
     ...
     # oh = self.size
      #ow = self.size
      output.append(img.resize((ww, hh), self.interpolation))
      # print output[0].size
    return output[0], output[1]
...
class RandomCrop(object):
  ...
  def __call__(self, imgA, imgB):
    ...
    if (w2<320)or(h2<320):
      ...
      if (h2<320):
          h2=320
      imgA=imgA.resize((w2, h2), Image.NEAREST)
      imgB=imgB.resize((w2, h2), Image.NEAREST)
    '''
    imgs = [imgA, imgB]
    output = []
    x1 = -1
